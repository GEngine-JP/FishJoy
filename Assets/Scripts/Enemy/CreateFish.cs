...
public class CreateFish : MonoBehaviour
{
    ...
    private float ItemTimeVal; //游戏物体计时器
...
//	[LuaCallCSharp]
        ...
        if (createManyFish >= 15)
        {
            ...
            else if (CreateMorden == 0 || CreateMorden == 1)
            {
                ...
                if (timeVals >= 0.2f)
                {
                    ...
                    GameObject itemGo = Instantiate(fishList[randomNum], CreateFishPlace[placeNum].position + new Vector3(0, 0, Random.Range(-2, 2)),
                        CreateFishPlace[placeNum].rotation);
                    itemGo.GetComponent<Fish>().cantRotate = true;
                    timeVals = 0;
                }
                else
    ...
    /// </summary>
    /// <param name="go"></param>
    private void CreateFishes(GameObject go)
    {
    ...
    //随机位置
    private Vector3 RandomPos(int randomNum)
    {
        ...
        {
            case 0:
                vPosition = new Vector3(-24, 1, Random.Range(-14f, 14f)); //-30  -  30
                break;
            case 1:
                vPosition = new Vector3(Random.Range(-24f, 24f), 1, 14); //60 - 120
                break;
            case 2:
                vPosition = new Vector3(24, 1, Random.Range(-14f, 14f)); //150-210
                break;
            case 3:
                vPosition = new Vector3(Random.Range(-24f, 24f), 1, -14); //-60-  -120
                break;
